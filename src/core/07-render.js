...
function drawWeld(g,p,T){
  const col="var(--weld)", x=T.X(p.x),y=T.Y(p.y), lx=T.X(p.lx),ly=T.Y(p.ly);
...
function drawPrim(g,p,T,redSet){
  switch(p.t){
    ...
    case "t":{const red=p.tag&&redSet&&redSet.has(p.tag);const n=svgel("text",{x:T.X(p.x),y:T.Y(p.y),"font-size":p.size||11,"text-anchor":p.anchor||"middle",fill:red?"var(--err)":(p.c==="label"?"var(--ink)":STY[p.c]?STY[p.c].s:"var(--ink)"),"font-weight":p.weight||"normal"},g);if(p.dy)n.setAttribute("dy",p.dy);if(p.rot)n.setAttribute("transform",`rotate(${p.rot} ${T.X(p.x)} ${T.Y(p.y)})`);n.textContent=p.s;break;}
