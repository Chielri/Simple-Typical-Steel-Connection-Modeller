...
GEO["BB-FIN"] = function(st){
  const A=[], B=[], fg=finGeom(st), {sec,prim,b,la,bp,hp,yc,ys,xs,baseX,ftip,outside,d0}=fg;
  ...
  // dimensions
  chainV(A, xs[0], -((prim.b+prim.tw)/2 + 32 + xs[0]), [yc+hp/2].concat(ys).concat([yc-hp/2]).sort((p,q)=>q-p), "p1");
  dimV(A, xb0+Lsec, yc-sec.h/2, yc+sec.h/2, 34, String(Math.round(sec.h)));
  dimH(A, yc-sec.h/2-26, baseX, baseX+g, -20, String(g), "gap");
  if(outside) dimH(A, yc+hp/2+22, 0, ftip, 18, String(Math.round(ftip)), "proj"); // plate cantilever over the flange
  if(st.n2>1) chainH(A, yc+hp/2+(outside?40:22), 18, xs, "p2");
  ...
  label(A, bp+12, yc-hp/2-6, [plCallout(st.tp,bp,hp,st.grade)+(outside?"  (extended)":""),
